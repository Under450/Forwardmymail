...
exports.stripeWebhook = onRequest(async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      // Idempotency: skip if this session was already processed (Stripe replays)
      const dupTxn = await db.collection('transactions')
      ...
      if (!customerId) {
        let existingCustomers = await db.collection('customers')
          .where('email', '==', customerEmail)
          .limit(1)
          .get();
        if (existingCustomers.empty && customerEmail && customerEmail !== customerEmail.toLowerCase()) {
      ...
      if (amount === 125)      { packageType = 'Personal Mailbox';    creditsToAdd = 0; }
      else if (amount === 175) { packageType = 'Business Address';    creditsToAdd = 0; }
      else if (amount === 225) { packageType = 'Registered Office';   creditsToAdd = 0; }
      ...
      await customerRef.update({
        ...
        ...(packageType !== 'Credit Pack' ? { package: packageType } : {}),
      ...
      try {
        ...
        if (creditsToAdd > 0) {
          ...
          emailHtml = buildPackageEmail(emailName, emailCompany, packageType, amount, finalCustomerData.mailboxId);
        }
...
exports.onCustomerCreated = onDocumentCreated('customers/{customerId}', async (event) => {
  try {
    ...
    // Server-side fallback: generate a unique mailbox ID if the client never did
    if (!mailboxId) {
      const usedIds = new Set();
      (await db.collection('customers').select('mailboxId').get())
        .forEach(d => { const m = d.data().mailboxId; if (m) usedIds.add(m); });
      do { mailboxId = 'FMM-' + Math.floor(10000 + Math.random() * 90000); }
      while (usedIds.has(mailboxId));
      await event.data.ref.update({ mailboxId, mailboxIdGenerated: admin.firestore.FieldValue.serverTimestamp() });
      console.log(`Generated mailbox ID ${mailboxId} server-side for ${event.params.customerId}`);
    }
...
exports.diditWebhook = onRequest(async (req, res) => {
  ...
  try {
    ...
    // Didit V3 dashboard has a known bug: displayed signing secret does NOT match
    // the secret used for signatures. Until they fix it, fall back to IP-based
    // verification: Didit delivers webhooks from static IP 18.203.201.92.
    // (Source: https://docs.didit.me/integration/webhooks)
    if (!verified) {
      const xff = req.headers['x-forwarded-for'] || '';
      const callerIp = String(xff).split(',')[0].trim() || req.ip || '';
      const isDiditIp = callerIp === '18.203.201.92';
      const ua = String(req.headers['user-agent'] || '');
      const isDiditUA = ua.startsWith('DiditWebhook/');

      console.warn('[diditWebhook] signature mismatch; fallback to IP+UA check —',
        'callerIp:', callerIp, 'UA:', ua,
        'sig present V2:', !!sigV2, 'raw:', !!sigRaw, 'simple:', !!sigSimple);

      if (isDiditIp && isDiditUA) {
        verified = true;
        verifiedVia = 'ip-allowlist-fallback';
      } else {
        return res.status(401).send('Invalid signature and untrusted IP');
      }
    }
    ...
    // Check customer exists before attempting update (test webhooks use fake UID)
    const customerRef = db.collection('customers').doc(customerId);
    const customerSnap = await customerRef.get();
    if (!customerSnap.exists) {
      console.warn('[diditWebhook] customer doc not found, acking only (test webhook?):', customerId);
